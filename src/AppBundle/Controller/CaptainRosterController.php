...
class CaptainRosterController extends FOSRestController
{
    ...
    public function getRoster(Request $request)
    {
        ...
        if($this->firebaseService->isAuthorized($authUser, UserType::$USER_TYPE_ADMIN)) {
            $userSeasons = $this->getDoctrine()->getRepository('AppBundle:UserSeasonEntity')->findBy(array('seasonId' => 5));
            $manualUserSeasons = $this->getDoctrine()->getRepository('AppBundle:ManualUserSeasonEntity')->findBy(array('seasonId' => 5));
